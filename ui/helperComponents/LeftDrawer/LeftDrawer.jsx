...
export default function LeftDrawer({ props }) {
  ...
  const userTopicsLists = useSelector((state) => state.topics.userTopics);
  ...
  const checkDuplication = (topicItem) => {
    const check = userTopicsLists.some(
      (item) => item.topic.id === topicItem.id
    );
    return !check;
  };
  ...
  return (
    <>
      {!userState ? (
        ...
        <div
          className={`${styles.leftNav} ${
            toggleKey ? styles.leftNavExpand : styles.leftNavCollapse
          }`}
        >
          <div className={styles.logoDiv}>
            {toggleKey && (
              <div
                className={styles.toggleCrossBtn}
          ...
          <div className={styles.addtopic}>
            <button onClick={() => setAddTopicModal(true)}>
              <span>
                <Image
                  src={plus}
                  alt="plus_icon"
          ...
          <div className={styles.yourtopics}>
            <div className={styles.yourtopicsheading}>
              <span>
                <Image
                  src={trending}
                  alt="plus_icon"
            ...
            <div className={styles.topicslist}>
              {userTopicsLists &&
                userTopicsLists.map((item) => (
                  <span
                    key={item.topic.id}
                    onClick={() => {
                      setCurrentChatTopicItem(item);
                      setTogglekey(false);
                    }}
                  >
                    {item.topic.name}
          ...
          <div className={styles.trendingtopics}>
            <div className={styles.trendingtopicsheading}>
              <span>
                <Image
                  src={trending}
                  alt="plus_icon"
            ...
            <div className={styles.trendingtopicslist}>
              {trendingTopicsLists &&
                trendingTopicsLists.map((item) => (
                  <span key={item.id} onClick={() => setTopicItem(item)}>
                    {item.name}
                  </span>
                ))}
            </div>
      ...
      {modalIsOpen && (
        <ConfirmModal
          props={{ modalIsOpen, closeModal, topicItem, user: user || null }}
        />
      )}
      {declineModal && (
        <DeclineModal props={{ declineModal, setDeclineModal }} />
      )}
      {addTopicModal && (
        <AddTopicModal props={{ addTopicModal, setAddTopicModal }} />
      )}
    </>
  );
}
